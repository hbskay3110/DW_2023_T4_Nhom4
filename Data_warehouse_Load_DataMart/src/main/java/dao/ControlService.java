...
public class ControlService {
	...
	public static String getStatusAggregateToday(int id) {
		return controlJdbi.withHandle(handle -> {
			return handle
					.createQuery("SELECT F.status FROM data_configs AS C "
	...
	public static int addStatus(int id, String note, String status) {
		...
		return controlJdbi.withHandle(handle -> {
			return handle.createUpdate(
					...
					.bind("id", id).bind("note", note).bind("status", status).bind("dateRun", date)
					.bind("created_by_modul", module).execute();
		});
	}
	...
	public static DataConfig getDataConfig(int id) {
		...
		return controlJdbi.withHandle(handle -> {
			return handle.createQuery(query).bind(0, id).mapToBean(DataConfig.class).findFirst().orElse(null);
		});
	}
